...
- ✅ **v1.1 Automatic Publishing** — Phases 4-9 (shipped 2026-04-17)
...
- [x] **Phase 7: Carousel Publishing (IG + FB)** — Completed 2026-04-17 (3 E2E tests pass, 1 bug fix wait 30→45s, 57 nodes deployed)
  Plans:
  - [x] 07-01-PLAN.md — Supabase schema extension + carousel guard removal + format branch + carousel session save
  - [x] 07-02-PLAN.md — IG carousel 3-step chain (7 nodes) + FB carousel attached_media chain + WA notification + Sheets log (7 nodes)
  - [x] 07-03-PLAN.md — Deploy to n8n-azure + E2E tests (5-slide, 3-slide, single-post regression)

#### Phase 8: Scheduling

- [x] **Phase 8: Scheduling** — Completed 2026-04-17 (Wizard PASO 6 + n8n scheduling gate, 60 nodes, 3 bugs fixed during E2E)
  Plans:
  - [x] 08-01-PLAN.md — Wizard scheduling prompt (PASO 6) + UTC conversion + n8n scheduling gate (3 nodes) + Supabase publish_at
  - [x] 08-02-PLAN.md — Deploy to n8n-azure + Azure min-replicas check + E2E tests (immediate + scheduled + past-time guard)

#### Phase 9: Error Hardening + Hashtags + Token Alerts

- [x] **Phase 9: Error Hardening + Hashtags + Token Alerts** — Completed 2026-04-17 (73 nodes deployed, 4 bugs fixed during E2E, error handler verified end-to-end)
  Plans:
  - [x] 09-01-PLAN.md — SAS regeneration (delete permission) + hashtag extraction + IG first-comment nodes
  - [x] 09-02-PLAN.md — Error handler subgraph: onError wiring + Parse Meta Error + WA alerts + Sheets fail log + blob cleanup
  - [x] 09-03-PLAN.md — Deploy to n8n-azure + E2E tests (happy path + error path + token expired + blob cleanup)
